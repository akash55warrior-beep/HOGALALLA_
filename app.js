...
/* ---------- Buy Now → WhatsApp ----------
   Since real checkout/payment isn't connected yet, "Buy Now" opens
   WhatsApp with the order details pre-filled — customer just hits
   send, no typing needed. Your number is set in config.js. */
function buyNowWhatsApp(productId, size, color, qty) {
  qty = qty || 1;
  ...
  if (typeof WHATSAPP_NUMBER === 'undefined' || !WHATSAPP_NUMBER) {
    showToast('Add your WhatsApp number in config.js first');
    return;
  }

  const colorIdx = p.colors ? p.colors.indexOf(color) : -1;
  const colorLabel = colorIdx === 0 ? 'Black' : (colorIdx === 1 ? 'Blue' : '');
  const link = window.location.origin + '/product.html?id=' + p.id;

  const lines = [
    `Hi HOGALALLA! I'd like to order:`,
    ``,
    `*${p.name}*`,
    `Size: ${size}${colorLabel ? ' | Colour: ' + colorLabel : ''}`,
    `Qty: ${qty}`,
    `Price: ${formatINR(p.price * qty)}`,
    ``,
    `Product link (photo + details): ${link}`
  ];
